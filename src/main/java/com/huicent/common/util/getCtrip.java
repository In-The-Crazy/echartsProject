...
public class getCtrip {
	...
	//String flightString = "MF858,SC8749,MF847,MF869,SC4956,MF8050,CZ5024,SC4968,MF8528,CZ5508";
	String flightString = "SC8749,SC4956,MF8050,CZ5024,SC4968,MF8528,CZ5508";

	public String getMaxMinPrice(String dcity, String acity, String flightDate, String startDate, String endDate) throws ParseException {
		Jedis jedis = null;
		...
		for(int l = 0;l < ss.length;l++){
			...
			List<String> listKeysMax = new ArrayList<String>();
			List<String> listKeysMin = new ArrayList<String>();
			for(int i = 0;i <= 30 ;i++){
				...
				for(int k = 0;k< keys.length;k = k + min){
					...
					if(!"".equals(flghtData) && null != flghtData){
						String[] flghtDataS = flghtData.split(",");
						flightTime = flghtDataS[3];
						int price = Integer.parseInt(flghtDataS[1]);
						if(price > maxPrice){
							maxPrice = price;
						}
						if(price < minPrice){
				...
				if(minPrice == 10000) minPrice = 0;
				listKeysMax.add(rDate + "," + String.valueOf(maxPrice) + "," + flightTime);
				listKeysMin.add(rDate + "," + String.valueOf(minPrice) + "," + flightTime);
				dateLong = dateLong + 86400000;
			}
			mapMax.put(ss[l], listKeysMax);
			mapmin.put(ss[l], listKeysMin);
		}
		map.put("max", mapMax);
		map.put("min", mapmin);
	...
	public String getForwardFight(String dcity, String acity, String startDate, String endDate, String weekDays) throws ParseException {
		...
		Map map = new HashMap();
		//String s = jedis.get(dcity + "-" + acity + endDate);
		String s = flightString;
		String[] ss = s.split(",");
		for(int l = 0;l < ss.length;l++){
			List<String> listKeys = new ArrayList<String>();
	...
	public String getHistoryFight(String dcity, String acity, String flightDate, String startDate, String endDate, String minute) throws ParseException{
		Jedis jedis = null;
		jedis = RedisUtil.getJedis();
		Map map = new HashMap();
		int min = 1;
...
//		}
		...
		System.out.println(a.getMaxMinPrice("TAO", "XMN", "2019-03-20", "2019-03-09", "2019-03-10"));
